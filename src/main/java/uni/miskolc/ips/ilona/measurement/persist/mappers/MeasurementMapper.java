...
public interface MeasurementMapper {
  /**
   * Selects all measurements from the database. It returns an empty collection, if no measurement
   * has been found.
   *
   * @return a collection of measurements, if there is no measurements then it returns with an empty
   *     collection.
   */
  Collection<Measurement> selectMeasurements();

  /**
   * Selects a position by an ID string from the database. Returs an empty collection if no
   * measurement has been found.
   *
   * @param id A string of the id wanted position.
   * @return a collection of measurements, if there is no measurements that fit the selection, then
   ...
   *
   * @param measId An ID string of the measurement.
   ...
   *
   * @param timestamp A timestamp of the measurement that needs to be deleted.
   */
   ...
   *
   * @param measurement The saved measurement.
   */
  void insertMeasurement(Measurement measurement);

  /**
   * Inserts data into a measurement object, based on a measurement from bluetooth.
   *
   * @param btDeviceId The ID of the measuring bluetooth object.
   * @param measId The ID sting of the measurement.
   */
  void insertBTDevice4Measurement(
      @Param("btDeviceId") String btDeviceId, @Param("measId") String measId);

  /**
   * Inserts data into a measurement object, based on a meaurement from WIFI-RSSI.
   *
   * @param ssid Id of the measuring RSSI object.
   * @param rssi A negative double in dB.
   * @param measId The ID string of the measurement.
   */
  void insertWiFiRSSI4Measurement(
      @Param("ssid") String ssid, @Param("rssi") double rssi, @Param("measId") String measId);
