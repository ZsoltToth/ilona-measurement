...
public class MeasurementDAOIntegrationTest extends SetupIntegrationTest {
  ...
  public void setUp() {
    ...
    try {
      ...
      this.dao =
          new MySQLMeasurementDAO(
              MYBATIS_CONFIG_FILE, HOST, PORT, DATABASE, USER, PASSWORD, positionDAO);
  ...
  public void testMapperResult() throws Exception {
    ...
    for (Measurement meas : measurements) {
      meas.setRfidtags(
          new RFIDTags(measurementMapper.selectRFIDTagsForMeasurement(meas.getId().toString())));
      meas.setBluetoothTags(
          new BluetoothTags(measurementMapper.selectBTTagsForMeasurement(meas.getId().toString())));
      List<Map<String, Double>> mysqlwifis =
          measurementMapper.selectWiFiRSSIForMeasurement(meas.getId().toString());
  ...
  public void test() throws Exception {
    ...
    try {
      ...
      bttags.setTags(
          measurementMapper.selectBTTagsForMeasurement("43c6270b-2d12-49f3-ac8f-0d09d55fcf61"));

      Set<byte[]> rfidTags =
          measurementMapper.selectRFIDTagsForMeasurement("43c6270b-2d12-49f3-ac8f-0d09d55fcf61");
      ...
      measurementBuilder.setbluetoothTags(
          new BluetoothTags(new HashSet<>(Arrays.asList("bt1", "bt2", "bt3"))));
      ...
      for (String wifi : wifirssi.keySet()) {
        measurementMapper.insertWiFiRSSI4Measurement(
            wifi, wifirssi.get(wifi), newMeas.getId().toString());
      }
  ...
  @Test
  public void testReadMeasurement() {
    Measurement expected = new Measurement();
    expected.setId(UUID.fromString("4304fe32-8028-4830-bebc-dd1d535e5cfd"));
    Measurement actual =
        dao.readMeasurement(UUID.fromString("4304fe32-8028-4830-bebc-dd1d535e5cfd"));
  ...
  public void testUpdateMeasurement() throws InsertionException, RecordNotFoundException {
    ...
    Measurement actual =
        dao.readMeasurement(UUID.fromString("59d46ae9-e0c8-48d0-b14a-503ed414b7cc"));
    Assert.assertEquals(measurement, actual);
